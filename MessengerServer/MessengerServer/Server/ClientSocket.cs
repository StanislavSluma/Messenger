using MessangerServer.Application.Application;
        ...
        public async Task ListenAsync()
        {
            try
            {
                while (true)
                {
                    ...
                    bool jwt_trigger = false;
                    if (tag == "SignUp?" || tag == "SignIn?")
                    {
                        jwt_trigger = true;
                    }
                    else
                    {
                        jwt_trigger = jwt.ValidateToken(parse_request[1]);
                    }
                    if (jwt_trigger)
                    {
                        switch (tag)
                        {
                            case "SignUp?":
                                {
                                    ...
                                    {
                                        response = RequestSerializer.Serialize("Succes?", jwt.GenerateToken(IsUser), IsUser);
                                        this.user = IsUser;
                                    }
                            ...
                            case "GetChatMessages?":
                                {
                                    int chatId = int.Parse(parse_request[2]);
                            ...
                            case "GetUsersByName?":
                                {
                                    string user_name = parse_request[2];
                            ...
                            case "CreateChat?":
                                {
                                    string chat_name = parse_request[2];
                            ...
                            case "AddUserToChat?":
                                {
                                    ...
                                    Chat? chat = JsonSerializer.Deserialize<Chat>(parse_request[3]);
                            ...
                            case "DeleteUserFromChat?":
                                {
                                    string response = "";
                                    Chat chat = JsonSerializer.Deserialize<Chat>(parse_request[2]);
                                    User deleted_user = JsonSerializer.Deserialize<User>(parse_request[3]);
                            ...
                            case "AddMessage?":
                                {
                                    string mess = parse_request[2];
                                    string usersId_str = parse_request[3];
                            ...
                            case "DeleteMessage?":
                                {
                                    int messId = int.Parse(parse_request[2]);
                                    Chat chat = JsonSerializer.Deserialize<Chat>(parse_request[3]);
                            ...
                            case "EditMessage?":
                                {
                                    Message mess = JsonSerializer.Deserialize<Message>(parse_request[2]);
                                    List<int> usersId = JsonSerializer.Deserialize<List<int>>(parse_request[3]);
                            ...
                            case "LeaveChat?":
                                {
                                    int chatId = JsonSerializer.Deserialize<int>(parse_request[2]);
                    ...
                    {
                        await RR_writer.WriteLineAsync("NotAuthorized");
                        await RR_writer.FlushAsync();
                    }
                }
            }
